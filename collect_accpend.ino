//data collection for calibration of pendulum accelerometers
//Read sensor data and print to serial monitor
//Values recorded: raw acceleromter readings for upper and lower pendulum accelerometers, potentiometer reading
//Use with collect_accpend_to_csv.py
...
void loop() {
  //read
  read_adxl(&device_upper);   //raw
  read_adxl(&device_lower);   //raw
  pot_ang = angle();          //deg

  //print adxls
  print_adxl(&device_upper);
  print_adxl(&device_lower);
  Serial.print(pot_ang);
  Serial.println(",");
}
